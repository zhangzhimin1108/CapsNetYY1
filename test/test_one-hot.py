...
for name in names:
    names = ['HCT116']
    name = names[0]
    m = np.load('D:/PycharmProjects/pythonProject/CapsNetYY1/feature encoding/one-hot/HCT116_test_one.npz')
    X_en_tes, X_pr_tes, y_tes = m['X_1_tes_one'], m['X_2_tes_one'], m['y_tes_one']
    y_tes_1=m['y_tes_one']
    y_tes= utils.to_categorical(y_tes, 2)
    pre_test_y = model.predict([X_en_tes, X_pr_tes])
    ...
    aupr = auc(rec, pre)
    y_pred_1 = model.predict([X_en_tes, X_pr_tes]).argmax(axis=1)
