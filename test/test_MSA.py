...
from modelMSA import get_model
import numpy as np
import pandas as pd
...
auc_score = []
model=get_model()
model.load_weights("D:/PycharmProjects/pythonProject/CapsNetYY1/mode_weight/HCT116_weight/HCT116Model_MSA.h5")
names = ['HCT116']
...
for name in names:
    names = ['HCT116']
    name = names[0]
    m = pd.read_csv('D:/PycharmProjects/pythonProject/CapsNetYY1/feature encoding/MSA/HCT116_test.csv', header=None)
    data = np.array(m).reshape(3594, 675, 1)
    Xtes_1 = data[0:1797]
    Xtes_2 = data[1797:3594]
    y_tes_1 = pd.read_csv('D:/PycharmProjects/pythonProject/YY1_data/HCT116labeltest.txt', header=None)
    ...
    plt.plot(fpr, tpr, color=coloer_list[0], lw=lw, label='MSA' + ' (AUC=%0.4f)' % auc_roc)
