...
from modelEIIP import get_model
import numpy as np
import pandas as pd
...
auc_score = []
model=get_model()
model.load_weights("D:/PycharmProjects/pythonProject/CapsNetYY1/mode_weight/HCT116_weight/HCT116Model_EIIP.h5")
names = ['HCT116']
...
for name in names:
    names = ['HCT116']
    name = names[0]
    m = pd.read_csv('D:/PycharmProjects/pythonProject/CapsNetYY1/feature encoding/EIIP/HCT116seq1_test.csv', header=None)
    n = pd.read_csv('D:/PycharmProjects/pythonProject/CapsNetYY1/feature encoding/EIIP/HCT116seq2_test.csv', header=None)
    Xtes_1 = np.array(m).reshape(1797, 506, 1)
    Xtes_2 = np.array(n).reshape(1797, 506, 1)
