...
names = ['HCT116',  'K562']
name=names[0]
m=pd.read_csv('D:/PycharmProjects/pythonProject/CapsNetYY1/feature encoding/PseEIIP/HCT116seq1train.csv',header=None)
n=pd.read_csv('D:/PycharmProjects/pythonProject/CapsNetYY1/feature encoding/PseEIIP/HCT116seq2train.csv',header=None)
X_1=np.array(m).reshape(4192,64,1)
X_2=np.array(n).reshape(4192,64,1)
...
for i,(train, test) in enumerate(kfold.split(y_tra)):
    print('\n\n%d'%i)
    #print(i,(train,test))
    path = 'D:/PycharmProjects/pythonProject/CapsNetYY1/PseEIIP_h5/%sModel%d.h5' % (name, i)
    ...
    history = model.fit([X_1[train], X_2[train]], y_tra[train], validation_data=([X_1[test], X_2[test]], y_tra[test]), epochs=200,
                        batch_size=64,callbacks=[checkpoint, back], verbose=2)
