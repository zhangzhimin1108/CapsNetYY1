...
names = ['HCT116',  'K562']
name=names[0]
m=pd.read_csv('D:/PycharmProjects/pythonProject/CapsNetYY1/feature encoding/EIIP/HCT116seq1_train.csv',header=None)
n=pd.read_csv('D:/PycharmProjects/pythonProject/CapsNetYY1/feature encoding/EIIP/HCT116seq2_train.csv',header=None)
X_1=np.array(m).reshape(4192,506,1)
X_2=np.array(n).reshape(4192,506,1)
...
for i,(train, test) in enumerate(kfold.split(y_tra)):
    ...
    def get_model():
        K.clear_session()
        X1 = Input(shape=(506,1))
        X2 = Input(shape=(506,1))
        x1 = Conv1D(filters=64, kernel_size=5, strides=1, padding='same', kernel_initializer='he_normal',activation='relu')(X1)
        x1 = MaxPooling1D(pool_size=2, strides=2)(x1)
        x1 = BatchNormalization()(x1)
        x1 = Dropout(0.2)(x1)
        x1 = Conv1D(filters=32, kernel_size=3, strides=1, padding='same', kernel_initializer='he_normal',activation='relu')(x1)
        x1 = MaxPooling1D(pool_size=2, strides=2)(x1)
        x1 = BatchNormalization()(x1)
        x1 = Dropout(0.2)(x1)
        x2 = Conv1D(filters=64, kernel_size=5, strides=1, padding='same', kernel_initializer='he_normal',activation='relu')(X2)
        x2 = MaxPooling1D(pool_size=2, strides=2)(x2)
        x2 = BatchNormalization()(x2)
        x2 = Dropout(0.2)(x2)
        x2 = Conv1D(filters=32, kernel_size=3, strides=1, padding='same', kernel_initializer='he_normal',activation='relu')(x2)
        x2 = MaxPooling1D(pool_size=2, strides=2)(x2)
        x2 = BatchNormalization()(x2)
        x2 = Dropout(0.2)(x2)
    ...
    history = model.fit([X_1[train], X_2[train]], y_tra[train], validation_data=([X_1[test], X_2[test]], y_tra[test]), epochs=200,
                        batch_size=32,callbacks=[checkpoint, back], verbose=2)
