...
auc_savepath = 'D:/PycharmProjects/pythonProject/YY1_zzm/NAC/HCT116/roc.png'
...
for i,(train, test) in enumerate(kfold.split(y_tra)):
    ...
    back = EarlyStopping(monitor='val_loss', patience=20, verbose=2, mode='auto')
    model = None
    model = get_model()
    model.summary()
    ...
    plt.plot(fpr, tpr, color=coloer_list[i], lw=lw, label='ROC fold' + str(i + 1) + '(AUC=%f)' % auc_roc)
...
plt.plot(fpr_mean, tpr_mean, color='r',lw=lw, label='Mean ROC(AUC= %f)' %auc_mean)
plt.legend(loc="lower right")
